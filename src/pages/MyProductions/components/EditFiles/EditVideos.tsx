...
const EditImages: React.FC = () => {
  ...
  const [fileList, setFileList] = useState<any>(
    productionSelected['videos_filename'],
  );

  useEffect(() => {
    setFileList(productionSelected['videos_filename']);
  }, [productionSelected]);
  ...
  const handleRemove = (file: any) => {
    api
      .delete(
        `productions/${productionSelected['key']}/video/${file?.filename}`,
  ...
  return (
    <>
      <UploadStyled
        action={`http://192.168.1.80:5000/api/v1/productions/${productionSelected['key']}/video`}
        headers={{ Authorization: 'Bearer ' + token }}
        listType="picture"
        fileList={
          productionSelected['videos_filename']?.length > 0 ? fileList : []
        }
