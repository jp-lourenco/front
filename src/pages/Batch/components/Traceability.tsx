    ...
    >
      {batch.type == 'batch' ? (
        ...
        <PanelStyled
          header={
            <HeaderPanel>
              <ImagePanel src="/Transformador.jpg" />
              <ContentPanel>
                <TitlePanel>Transformador</TitlePanel>

                <SubtitlePanel>{batch?.transformer}</SubtitlePanel>
              </ContentPanel>
            </HeaderPanel>
          }
          key="1"
        >
          {batch?.subbatch_code && (
            <ContainerText>
              <Label>Código do sublote:</Label>
              <Text>{batch?.subbatch_code}</Text>
            </ContainerText>
          )}
          {batch?.transformation_start && (
            <ContainerText>
              <Label>Início da tranformação:</Label>
              <Text>{moment(batch?.transformation_start).format('llll')}</Text>
            </ContainerText>
          )}
          {batch?.transformation_description && (
            <ContainerText>
              <Label>Descrição da tranformação:</Label>
              <Text>{batch?.transformation_description}</Text>
            </ContainerText>
          )}
          {batch?.transformation_end && (
            <ContainerText>
              <Label>Final da tranformação:</Label>
              <Text>{moment(batch?.transformation_end).format('llll')}</Text>
            </ContainerText>
          )}
          {batch?.amount_transformed && (
            <ContainerText>
              <Label>Quantidade transformada:</Label>
              <Text>{batch?.amount_transformed}kg</Text>
            </ContainerText>
          )}
          <ContainerText>
            <Label>Transformador enviou:</Label>
            <Text>{moment(batch?.history[0]?.date).format('llll')}</Text>
          </ContainerText>
        </PanelStyled>
      )}
      {histories
        ?.filter((history) => !history[0].transition.includes('tentou'))
